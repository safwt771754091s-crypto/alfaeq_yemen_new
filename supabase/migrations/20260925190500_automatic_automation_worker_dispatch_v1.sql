-- Automatically wake the automation worker when an event enters the durable outbox.
-- The worker performs its own shared-secret authorization and claims events atomically.
...
for each row execute function private.trigger_automation_worker();

revoke execute on function private.trigger_automation_worker() from public, anon, authenticated;
