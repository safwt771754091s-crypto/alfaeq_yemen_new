...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final List<String> _roles = ['مستخدم', 'تاجر', 'مدير النظام'];
  ...
  void _login() {
    if (_formKey.currentState!.validate()) {
      ...
      Future.delayed(const Duration(seconds: 2), () {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم تسجيل الدخول بنجاح بصلاحية: $_selectedRole'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('تسجيل الدخول وصلاحيات التطبيق'),
        centerTitle: true,
