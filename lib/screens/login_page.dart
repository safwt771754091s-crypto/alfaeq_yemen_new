...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _pickLocation() async {
    final point = await Navigator.of(context).push<LatLngSelection>(
      MaterialPageRoute(builder: (_) => const LocationPickerPage()),
    );
    if (point != null && mounted) {
      setState(() => _pendingLocation = GeoPoint(point.latitude, point.longitude));
    }
  }
                  ...
                  Card(elevation: 0, child: Padding(
                    ...
                    child: Form(key: _formKey, child: Column(children: [
                      ...
                      if (_register) ...[
                        ...
                        Card(elevation: 0, color: _pendingLocation == null ? null : Theme.of(context).colorScheme.primaryContainer, child: ListTile(
                          leading: const Icon(Icons.location_on_outlined),
                          title: Text(_pendingLocation == null ? 'تحديد الموقع مطلوب' : 'تم تحديد موقع الحساب'),
                          subtitle: Text(_pendingLocation == null ? 'حدد موقعك من الخريطة أو استخدم موقع الهاتف.' : '${_pendingLocation!.latitude.toStringAsFixed(5)}, ${_pendingLocation!.longitude.toStringAsFixed(5)}'),
                          trailing: IconButton(onPressed: _loading ? null : _pickLocation, icon: const Icon(Icons.map_outlined)),
                      ...
                      if (!_register) ...[
                        ...
                        SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: _loading ? null : _forgotPassword, icon: const Icon(Icons.lock_reset_rounded), label: const Text('نسيت كلمة المرور؟', style: TextStyle(fontWeight: FontWeight.w800)))),
                      ],
                      const SizedBox(height: 12),
...
class LatLngSelection {
  final double latitude;
  final double longitude;
  const LatLngSelection(this.latitude, this.longitude);
}
