...
class _DispatchCenterPageState extends State<DispatchCenterPage> {
  ...
  Future<void> _setLocation(QueryDocumentSnapshot<Map<String, dynamic>> doc) async {
    ...
    if (latitude == null || longitude == null || latitude.abs() > 90 || longitude.abs() > 180) { _message('الإحداثيات غير صحيحة.'); return; }
  ...
  Future<void> _assign(QueryDocumentSnapshot<Map<String, dynamic>> doc) async {
    ...
    if (destination is! GeoPoint) { _message('حدد موقع التسليم أولاً.'); return; }
    setState(() => _busy = true);
    try {
      final candidate = await _dispatch.assignBestDriver(orderId: doc.id, destination: destination, actorUid: user.uid, actorRole: await _auth.role());
  ...
  @override
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: FutureBuilder<bool>(future: _allowed(), builder: (context, access) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('التوزيع الذكي')), body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(padding: const EdgeInsets.all(16), itemCount: docs.length, itemBuilder: (context, index) {
          final doc = docs[index]; final data = doc.data(); final hasLocation = data['deliveryLocation'] is GeoPoint;
          return Card(elevation: 0, margin: const EdgeInsets.only(bottom: 12), child: ListTile(
            ...
            subtitle: Text('${data['address'] ?? '—'}\n${hasLocation ? 'موقع التسليم جاهز' : 'يحتاج موقع تسليم'}'), isThreeLine: true,
