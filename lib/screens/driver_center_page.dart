...
class _DriverCenterPageState extends State<DriverCenterPage> {
  ...
  Future<void> _toggleOnline(bool value) async {
    ...
    try {
      ...
      if (value) {
        await _startLiveLocation(user.uid);
      } else {
  ...
  Future<void> _startLiveLocation(String uid) async {
    await _stopLiveLocation();
    await LocationService.requireCurrentPosition();
    const settings = LocationSettings(
      accuracy: LocationAccuracy.high,
      distanceFilter: 20,
    );
    _locationSubscription = Geolocator.getPositionStream(locationSettings: settings).listen(
      (position) => _publishLiveLocation(uid, position),
      onError: (Object error) => _message('توقف تتبع الموقع: $error'),
    );
  }
  ...
  Future<void> _publishLiveLocation(String uid, Position position) async {
    ...
    try {
      ...
      final activeOrders = await _db
          ...
          .where('deliveryStatus', whereIn: const ['assigned', 'picked_up', 'out_for_delivery'])
          .limit(10)
          .get();
      ...
      // A transient network failure must not stop the location stream.
    }
  }
  ...
  Future<void> _updateLocation() async {
    ...
    try {
      ...
      await _ensureProfile(user.uid);
      await _db.collection('drivers').doc(user.uid).update({
        'currentLocation': LocationService.toGeoPoint(position),
        'lastSeenAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
      });
      _message('تم تحديث الموقع الحالي.');
    } catch (_) {
      _message('تعذر تحديد موقع المندوب. فعّل GPS واسمح بالوصول إلى الموقع.');
    }
  }
    ...
    try {
      ...
      await _db.runTransaction((tx) async {
        ...
        if (status == 'delivered') {
          ...
          tx.update(driverRef, {
            'activeOrderCount': active > 0 ? active - 1 : 0,
            'updatedAt': FieldValue.serverTimestamp(),
          });
        }
      });
      ...
      _message('تعذر تحديث المهمة: ${e.message ?? e.code}');
    ...
    return Directionality(
      ...
      child: FutureBuilder<bool>(
        ...
        builder: (context, access) {
          ...
          if (access.data != true || user == null) {
            return const Scaffold(body: Center(child: Text('مركز المندوب محمي للحسابات المعتمدة.')));
          }
                  ...
                  children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SwitchListTile(
                              ...
                              title: const Text('متاح لاستلام الطلبات'),
                              subtitle: Text(online ? 'متصل — التتبع الحي يعمل' : 'غير متصل'),
                              value: online,
                              onChanged: _busy || !approved ? null : _toggleOnline,
                            ),
                          ],
                        ),
                      ),
                    ...
                    Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(data['currentLocation'] is GeoPoint ? 'الموقع يتحدث تلقائياً عند الاتصال.' : 'لم يتم تسجيل موقع بعد.'),
                        trailing: const Icon(Icons.my_location),
                        onTap: approved ? _updateLocation : null,
                      ),
                    ),
                    const SizedBox(height: 12),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 10),
                                if (docs.isEmpty)
                                  const Text('لا توجد مهام مسندة حالياً.')
                                else
                                  ...docs.map((doc) => _DriverOrderTile(doc: doc, busy: _busy, onStatus: _setDeliveryStatus)),
                              ],
                            ),
                          ),
                        );
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        title: Text('طلب #${doc.id.substring(0, doc.id.length > 8 ? 8 : doc.id.length)}'),
