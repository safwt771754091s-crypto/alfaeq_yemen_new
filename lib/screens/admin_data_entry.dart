...
class _AdminDataEntryState extends State<AdminDataEntry> {
  ...
  Future<void> saveMerchant() async {
    ...
    if (user == null || merchantName.text.trim().isEmpty || merchantPhone.text.trim().isEmpty) return;
    ...
    try {
      final ref = await FirebaseFirestore.instance.collection('stores').add({
        ...
        'sectionId': sectionId,
        'status': 'approved',
        // Ownership is tied to the authenticated admin that created the record.
        // Never use a fake owner such as "admin-created" in production data.
        'ownerId': user.uid,
        'createdBy': user.uid,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تمت إضافة التاجر وحفظه في Firestore بملكية حساب حقيقي.')),
        );
      }
    } finally {
  ...
  Future<void> saveProduct() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null || storeId == null || productName.text.trim().isEmpty) return;

    final price = num.tryParse(productPrice.text.trim());
    final stock = int.tryParse(productStock.text.trim());
    if (price == null || price < 0 || stock == null || stock < 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تحقق من السعر والكمية قبل الحفظ.')),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تمت إضافة الصنف وحفظه في Firestore بملكية حقيقية.')),
        );
      }
    } finally {
      if (mounted) setState(() => saving = false);
