...
class _AdminDataEntryState extends State<AdminDataEntry> {
  ...
  @override
  void dispose() {
    for (final c in [_storeName, _phone, _address, _productName, _description, _price, _stock, _imageUrl]) c.dispose();
    super.dispose();
  }
  ...
  Future<void> _pickStoreLocation({LatLng? initial}) async {
    final result = await Navigator.push<LatLng>(
      context,
      MaterialPageRoute(builder: (_) => LocationPickerPage(initialLatitude: initial?.latitude, initialLongitude: initial?.longitude, title: 'تحديد موقع المتجر بدقة')),
    );
  ...
  Future<void> _createStore() async {
    ...
    try {
      ...
      final ref = await FirebaseFirestore.instance.collection('stores').add({
        'name': name, 'phone': phone, 'address': _address.text.trim(), 'sectionId': _sectionId, 'status': 'approved',
        'ownerId': user.uid, 'createdBy': user.uid, 'ownerType': 'platform_admin', 'location': point,
        'latitude': point.latitude, 'longitude': point.longitude, 'locationSource': 'admin_map_picker',
        'locationUpdatedAt': FieldValue.serverTimestamp(), 'createdAt': FieldValue.serverTimestamp(), 'updatedAt': FieldValue.serverTimestamp(),
      });
      setState(() { _selectedStoreId = ref.id; _selectedStoreSectionId = _sectionId; });
      _storeName.clear(); _phone.clear(); _address.clear(); _storeLocation = null;
    ...
    return Directionality(
      ...
      child: FutureBuilder<bool>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data != true) return const Scaffold(body: Center(child: Text('هذه الصفحة مخصصة لحساب الأدمن.')));
          return Scaffold(
            appBar: AppBar(title: const Text('البيانات الحقيقية — Firestore'), actions: [IconButton(tooltip: 'استيراد أصناف بالجملة', onPressed: _saving ? null : _openBulkImport, icon: const Icon(Icons.upload_file_outlined))]),
            body: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                const Card(child: ListTile(leading: Icon(Icons.cloud_done_outlined), title: Text('قاعدة البيانات الحقيقية', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('أي متجر أو صنف تحفظه هنا يُكتب مباشرة في Cloud Firestore، وليس بيانات تجريبية.'))),
                const SizedBox(height: 12),
                Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.upload_file_outlined)), title: const Text('استيراد الأصناف بالجملة', style: TextStyle(fontWeight: FontWeight.w900)), subtitle: const Text('Excel أو CSV — رقم المرجع، الاسم، الكمية، السعر، الصورة والبيانات الأخرى مع معاينة قبل الاعتماد.'), trailing: const Icon(Icons.chevron_left), onTap: _openBulkImport)),
                const SizedBox(height: 18),
                const Text('المتاجر الحقيقية', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900)), const SizedBox(height: 8), _stores(),
                const SizedBox(height: 18),
                const Text('إضافة متجر حقيقي', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900)), const SizedBox(height: 8),
                Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
                  DropdownButtonFormField<String>(initialValue: _sectionId, decoration: const InputDecoration(labelText: 'القسم'), items: [for (final s in appSections) DropdownMenuItem(value: s.id, child: Text(s.title))], onChanged: (v) => setState(() => _sectionId = v ?? _sectionId)),
                  ...
                  const SizedBox(height: 12),
                  Card(elevation: 0, child: ListTile(leading: Icon(_storeLocation == null ? Icons.location_off_outlined : Icons.location_on), title: Text(_storeLocation == null ? 'موقع المتجر غير محدد' : 'تم تحديد موقع المتجر'), subtitle: Text(_storeLocation == null ? 'اضغط لتحديد الموقع على الخريطة' : '${_storeLocation!.latitude.toStringAsFixed(6)}, ${_storeLocation!.longitude.toStringAsFixed(6)}'), trailing: const Icon(Icons.map_outlined), onTap: _saving ? null : () => _pickStoreLocation(initial: _storeLocation))),
                ...
                const SizedBox(height: 18),
                const Text('إضافة صنف حقيقي', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900)), const SizedBox(height: 8),
                Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
                ...
                const SizedBox(height: 18),
                const Text('الأصناف الموجودة فعلياً', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900)), const SizedBox(height: 8), _products(),
              ],
            ),
          );
        },
  ...
  Widget _stores() => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (context, snapshot) {
      ...
      return Column(children: docs.map((doc) {
        final data = doc.data(); final selected = _selectedStoreId == doc.id; final lat = data['latitude']; final lng = data['longitude'];
        return Card(child: ListTile(selected: selected, leading: Icon(data['location'] is GeoPoint ? Icons.location_on : Icons.location_off_outlined), title: Text('${data['name'] ?? 'متجر'}', style: const TextStyle(fontWeight: FontWeight.w900)), subtitle: Text('${data['status'] ?? 'pending'} • ${data['phone'] ?? ''}\n${lat is num && lng is num ? 'الموقع: ${lat.toStringAsFixed(5)}, ${lng.toStringAsFixed(5)}' : 'الموقع غير محدد'}'), isThreeLine: true, trailing: selected ? const Icon(Icons.check_circle) : const Icon(Icons.chevron_left), onTap: () => setState(() { _selectedStoreId = doc.id; _selectedStoreSectionId = data['sectionId'] as String?; }))); 
      }).toList());
    },
  );
  ...
  Widget _products() => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (context, snapshot) {
      ...
      return Column(children: visible.map((doc) {
        final data = doc.data();
        return Card(child: ListTile(leading: const Icon(Icons.inventory_2_outlined), title: Text('${data['name'] ?? 'صنف'}', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${data['description'] ?? ''}\nالمرجع: ${data['reference'] ?? 'غير محدد'} • المخزون: ${data['stock'] ?? 0} • الحالة: ${data['status'] ?? 'active'}'), isThreeLine: true, trailing: Text('${data['price'] ?? 0} ${data['currency'] ?? 'YER'}', style: const TextStyle(fontWeight: FontWeight.w900))));
      }).toList());
    },
  );
}
