...
class CartPage extends StatelessWidget {
  ...
  Future<void> _changeQuantity(String uid, String productId, int delta) async {
    ...
    await FirebaseFirestore.instance.runTransaction((tx) async {
      ...
      final current = (items[index]['quantityBase'] as num?)?.round() ??
          ((items[index]['quantity'] as num?) ?? 1).round() * unit.scale;
      ...
      if (next <= 0) {
        ...
        items[index]['quantity'] = unit.toDisplay(next);
      }
  ...
  Future<void> _remove(String uid, String productId) async {
    ...
    await FirebaseFirestore.instance.runTransaction((tx) async {
      ...
      tx.set(
        ref,
        {'ownerId': uid, 'items': items, 'currency': data['currency'] ?? 'YER', 'updatedAt': FieldValue.serverTimestamp()},
        SetOptions(merge: true),
      );
    });
  }
  ...
  Future<void> _checkout(BuildContext context, String uid, List<Map<String, dynamic>> items, num total, String currency) async {
              ...
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'الحي، الشارع، معلم قريب',
                  ),
                ),
                ...
                OutlinedButton.icon(
                  onPressed: () async {
                    final point = await Navigator.push<LatLng>(
                      context,
                      MaterialPageRoute(builder: (_) => const LocationPickerPage(title: 'تحديد موقع التوصيل')),
                    );
                    if (point != null) setState(() => deliveryPoint = point);
                  },
                  icon: Icon(deliveryPoint == null ? Icons.location_on_outlined : Icons.location_on),
                  label: Text(
                    deliveryPoint == null ? 'حدد موقع التوصيل على الخريطة' : 'تم تحديد موقع التوصيل',
                  ),
                ),
                const SizedBox(height: 14),
    ...
    if (address.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('حدد موقع التوصيل على الخريطة أو اكتب العنوان أولاً.')),
        );
      }
      return;
    }
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تعذر إنشاء الطلب: ${e.message ?? e.code} (code: ${e.code})')),
        );
      }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تعذر إنشاء الطلب: ${e.message ?? e.code}')),
        );
      }
    }
  }
  ...
  Future<Map<String, dynamic>> _createLocalOrderDraft({
    ...
    for (final item in items) {
      ...
      final base = (item['quantityBase'] as num?)?.toDouble() ??
          (((item['quantity'] as num?) ?? 1) * unit.scale).toDouble();
      final requested = base.round();
      final stockBase = ProductUnit.stockBase(p);
      if (requested <= 0 || requested > stockBase) {
        throw StateError('المخزون غير كافٍ للمنتج: ${p['name'] ?? productId}.');
      }

      final quantity = unit.toDisplay(requested);
        ...
        body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            for (final item in items) {
              ...
              final base = (item['quantityBase'] as num?)?.toDouble() ?? (((item['quantity'] as num?) ?? 0) * unit.scale).toDouble();
              final price = (item['price'] as num?) ?? 0;
              total += price * unit.toDisplay(base.round());
            }
            ...
            return ListView(
              ...
              children: [
                ...items.map((item) {
                  ...
                  final unit = ProductUnit.fromId((item['saleUnit'] ?? 'piece').toString());
                  final base = (item['quantityBase'] as num?)?.toDouble() ?? (((item['quantity'] as num?) ?? 1) * unit.scale).toDouble();
                  final quantity = unit.toDisplay(base.round());
                  final price = (item['price'] as num?) ?? 0;

                  final displayQty = quantity % 1 == 0 ? quantity.toStringAsFixed(0) : quantity.toStringAsFixed(2);
                  ...
                  return Card(
                    child: Padding(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text('$displayQty ${unit.label} × $price $currency/${unit.label} • المجموع: ${price * quantity} $currency'),
                        trailing: SizedBox(
                          width: 150,
                          child: Row(
                            ...
                            children: [
                              IconButton(
                                onPressed: productId.isEmpty ? null : () => _changeQuantity(user.uid, productId, -1),
                                icon: const Icon(Icons.remove_circle_outline),
                              ),
                              Text(displayQty, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
                              IconButton(
                                onPressed: productId.isEmpty ? null : () => _changeQuantity(user.uid, productId, 1),
                                icon: const Icon(Icons.add_circle_outline),
                              ),
                              IconButton(
                                onPressed: productId.isEmpty ? null : () => _remove(user.uid, productId),
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: () => _checkout(context, user.uid, items, total, currency),
