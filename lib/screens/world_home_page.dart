  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      Row(children: [
        ...
        const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('الفائق يمن', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w900)), Text('منصة واحدة للحياة والأعمال', style: TextStyle(color: Colors.black54, fontSize: 12))])),
...
class WorldSectionPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(
    appBar: AppBar(title: Text(section.title, style: const TextStyle(fontWeight: FontWeight.w900)), actions: [IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CartPage())), icon: const Icon(Icons.shopping_cart_outlined))]),
    body: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 28), children: [
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFE7F3EE), Color(0xFFEAF2F8)]), borderRadius: BorderRadius.circular(24)), child: Row(children: [Container(width: 56, height: 56, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(17)), child: Icon(_SectionCard.iconFor(section.icon), color: const Color(0xFF0B6E4F), size: 31)), const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(section.title, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(section.subtitle, style: const TextStyle(color: Colors.black54, height: 1.3))]))])),
      const SizedBox(height: 14),
    ...
    try {
      await FirebaseFirestore.instance.runTransaction((tx) async {
        ...
        if (itemIndex >= 0) {
          ...
          items.add({'productId': product.id, 'name': name, 'price': price, 'currency': data['currency'] ?? 'YER', 'quantity': 1, 'storeId': data['storeId'] ?? store.id, 'merchantId': data['merchantId'] ?? data['ownerId'] ?? '', 'ownerId': data['ownerId'] ?? '', 'imageUrl': data['imageUrl'] ?? data['image'] ?? '', 'addedAt': Timestamp.now()});
        }
        ...
        StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            return Column(children: products.map((product) {
              ...
              return ListTile(
                leading: imageUrl.isEmpty ? const CircleAvatar(backgroundColor: Color(0xFFE7F3EE), child: Icon(Icons.inventory_2_outlined, color: Color(0xFF0B6E4F))) : ClipRRect(borderRadius: BorderRadius.circular(10), child: Image.network(imageUrl, width: 48, height: 48, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const CircleAvatar(child: Icon(Icons.broken_image_outlined))),
                title: Text('${productData['name'] ?? 'صنف'}', style: const TextStyle(fontWeight: FontWeight.w800)),
                subtitle: Text('المتوفر: ${productData['stock'] ?? '—'}'),
                trailing: SizedBox(width: 150, child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [Expanded(child: Text(price is num ? '$price ${productData['currency'] ?? 'YER'}' : 'عند الطلب', textAlign: TextAlign.end, style: const TextStyle(fontWeight: FontWeight.w900))), IconButton(onPressed: () => _addToCart(context, product), tooltip: 'أضف للسلة', icon: const Icon(Icons.add_shopping_cart, color: Color(0xFF0B6E4F)))])),
...
class ServicesHubPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: ListView(padding: const EdgeInsets.all(16), children: [
    const Text('الخدمات', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900)),
    const SizedBox(height: 8),
    const Text('الخدمات والطلبات والدفع والتتبع من مكان واحد.', style: TextStyle(color: Colors.black54)),
    const SizedBox(height: 18),
    _tile(context, 'السلة والشراء', Icons.shopping_cart_outlined, const CartPage()),
    _tile(context, 'طلباتي', Icons.local_shipping_outlined, const MyOrdersPage()),
    _tile(context, 'المحافظ', Icons.account_balance_wallet_outlined, const WalletCenterPage()),
    _tile(context, 'ذكاء الفائق', Icons.auto_awesome, const AiAssistantPage()),
  ]));
  Widget _tile(BuildContext context, String title, IconData icon, Widget page) => Card(child: ListTile(leading: Icon(icon, color: const Color(0xFF0B6E4F)), title: Text(title, style: const TextStyle(fontWeight: FontWeight.w800)), trailing: const Icon(Icons.chevron_left), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => page))));
}

class WalletCenterPage extends StatelessWidget {
  const WalletCenterPage({super.key});
  @override
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('المحافظ')), body: ListView(padding: const EdgeInsets.all(16), children: const [
    Card(child: ListTile(leading: Icon(Icons.account_balance_wallet_outlined), title: Text('محفظة الفائق'), subtitle: Text('المحفظة الخاصة بالمنصة والحسابات المالية ستدار بصلاحيات آمنة.'))),
    Card(child: ListTile(leading: Icon(Icons.payments_outlined), title: Text('طرق الدفع'), subtitle: Text('الدفع عند الاستلام والكريمي والمحافظ النقدية وجيب.'))),
  ])));
}
...
class _AccountTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl, child: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.person)), title: Text(user?.displayName ?? 'مستخدم الفائق'), subtitle: Text(user?.email ?? user?.phoneNumber ?? 'حساب مسجل'))),
      const SizedBox(height: 12),
      Card(child: ListTile(leading: const Icon(Icons.shopping_cart_outlined), title: const Text('سلة التسوق'), trailing: const Icon(Icons.chevron_left), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CartPage())))),
      Card(child: ListTile(leading: const Icon(Icons.local_shipping_outlined), title: const Text('طلباتي'), trailing: const Icon(Icons.chevron_left), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MyOrdersPage())))),
      const SizedBox(height: 18),
      FilledButton.icon(onPressed: () async { await FirebaseAuth.instance.signOut(); }, icon: const Icon(Icons.logout), label: const Text('تسجيل الخروج')),
    ]));
  }
}
...
class _Info extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 6), Text(text, style: const TextStyle(color: Colors.black54))])));
}
