...
class _DeveloperPageState extends State<DeveloperPage> {
  ...
  String _role = '';
  String _appCheckStatus = 'غير مفحوص';
  ...
  Future<void> _checkAppIntegrity() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _appCheckStatus = token == null
            ? 'غير نشط — يلزم إعداد مزود App Check'
            : 'نشط ومصادق عليه';
      });
    } catch (_) {
      if (mounted) setState(() => _appCheckStatus = 'تعذر التحقق من App Check');
    }
  }
  ...
  Future<void> _runSecurityScan() async {
    ...
    try {
      ...
      counts.addAll({
        ...
        'orders': results[3].docs.length,
        'payments': results[4].docs.length,
      });
      ...
      final usersWithoutRole = users.where((d) => d.data()['role'] is! String || (d.data()['role'] as String).isEmpty).length;
      if (legacyStores > 0) {
        findings.add('يوجد $legacyStores متجر بملكية قديمة admin-created ويحتاج ربطه بحساب حقيقي.');
      }
      if (legacyProducts > 0) {
        findings.add('يوجد $legacyProducts منتج بملكية قديمة admin-created ويحتاج ربطه بمالك حقيقي.');
      }
      if (usersWithoutRole > 0) {
        findings.add('يوجد $usersWithoutRole مستخدم بلا role واضح في البيانات المفحوصة.');
      }
      if (_appCheckStatus != 'نشط ومصادق عليه') {
        findings.add('App Check غير نشط بالكامل لهذه البيئة؛ لا نعتبر الحماية مكتملة حتى تفعيل المزود في Firebase.');
      }
      if (findings.isEmpty) {
        findings.add('لم يظهر خلل حرج في العينة المفحوصة. الفحص الحالي حدّه 200 سجل لكل مجموعة.');
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (!_allowed) {
      return Directionality(
        ...
        child: Scaffold(
          appBar: AppBar(title: const Text('مركز المطور')),
          body: const Center(
            child: Padding(
              padding: EdgeInsets.all(24),
              child: Text(
                'هذه المنطقة محمية. لا توجد لديك صلاحية المطور.',
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('مركز المطور والأمن', style: TextStyle(fontWeight: FontWeight.w900)),
        ...
        body: ListView(
          ...
          children: [
            ...
            _sectionTitle('🛡️ الحماية السيبرانية'),
            ...
            const SizedBox(height: 14),
            _sectionTitle('🤖 الأتمتة والفحص'),
            Card(
              child: ListTile(
                leading: const Icon(Icons.radar_outlined),
                title: const Text('فحص أمني تلقائي للبيانات', style: TextStyle(fontWeight: FontWeight.w900)),
                subtitle: Text(_lastScan == null
                    ? 'يفحص عينة حقيقية من المستخدمين والمتاجر والمنتجات والطلبات والمدفوعات.'
                    : 'آخر فحص: ${_lastScan!.toLocal()}'),
                trailing: FilledButton.icon(
                  onPressed: _scanning ? null : _runSecurityScan,
                  icon: _scanning
            ...
            if (_counts.isNotEmpty) _dataOverview(),
            if (_findings.isNotEmpty) _findingsCard(),
            const SizedBox(height: 14),
            _sectionTitle('📜 سجل التدقيق'),
            _auditLogCard(),
            const SizedBox(height: 14),
            _sectionTitle('💻 تطوير التطبيق'),
            const _DevCard(icon: Icons.code, title: 'مراجعة الكود', subtitle: 'فحص المصدر الكامل يتم عبر CI قبل النشر.'),
            const _DevCard(icon: Icons.storage, title: 'فحص البيانات', subtitle: 'تحقق من الملكيات والأدوار والمؤشرات غير الطبيعية في البيانات الحقيقية.'),
            const _DevCard(icon: Icons.auto_awesome, title: 'مساعد التطوير والأتمتة', subtitle: 'الأمن، اكتشاف المخاطر، أتمتة الفحوصات، ومتابعة جودة النظام.'),
            const SizedBox(height: 14),
            _sectionTitle('👤 الحساب الحالي'),
            _DevCard(
              icon: Icons.person,
              title: _profile['name']?.toString() ?? 'حساب المستخدم',
              subtitle: 'UID: ${FirebaseAuth.instance.currentUser?.uid ?? '-'}\nالبريد: ${FirebaseAuth.instance.currentUser?.email ?? '-'}\nالدور: $_role',
            ),
          ],
        ),
      ),
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return Card(child: ListTile(leading: const Icon(Icons.error_outline), title: const Text('تعذر تحميل سجل التدقيق'), subtitle: Text('${snapshot.error}')));
        }
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Row(children: [const Icon(Icons.history), const SizedBox(width: 8), const Expanded(child: Text('آخر العمليات', style: TextStyle(fontWeight: FontWeight.w900))), Text('${filtered.length} من ${docs.length}')]),
              ),
              if (filtered.isEmpty)
  ...
  Widget _auditFilters(List<String> actions) {
    final selectedAction = actions.contains(_auditActionFilter) || _auditActionFilter == 'الكل' ? _auditActionFilter : 'الكل';
    return Padding(
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: () {
                  ...
                  setState(() { _auditActionFilter = 'الكل'; _auditResultFilter = 'الكل'; });
                },
          ...
          builder: (context, controller) => Material(
            ...
            child: ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    CircleAvatar(radius: 24, backgroundColor: status.color.withValues(alpha: .12), child: Icon(status.icon, color: status.color)),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_actionLabel(action), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Chip(label: Text(status.label))])),
                  ],
                ),
                ...
                Container(
                  ...
                  child: SelectableText(detailsText.isEmpty ? 'لا توجد تفاصيل تقنية.' : detailsText, style: const TextStyle(fontFamily: 'monospace', height: 1.5)),
                ),
  ...
  Widget _detailSection(String label, String value, IconData icon, {Color? valueColor}) {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurfaceVariant)), const SizedBox(height: 3), SelectableText(value, style: TextStyle(fontWeight: FontWeight.w800, color: valueColor))])),
          ],
        ),
      ),
    );
  ...
  ({String label, IconData icon, Color color}) _auditStatus(String result, String severity) {
    if (result == 'failed' || result == 'failure' || severity == 'critical') {
      return (label: 'فشل', icon: Icons.error, color: Colors.red);
    }
    if (result == 'warning' || severity == 'warning') {
      return (label: 'تحذير', icon: Icons.warning_amber, color: Colors.orange);
    }
  ...
  String _actionLabel(String action) => const {
        ...
        'developer_center_access': 'فتح مركز المطور',
      }[action] ?? action;
  ...
  Widget _heroCard() => Card(
        ...
        child: const Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 10),
            Text('مركز المطور الذكي', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)),
            SizedBox(height: 6),
            Text('تطوير • فحص • أتمتة • أمن سيبراني • حماية البيانات', style: TextStyle(color: Colors.white70)),
          ]),
        ),
      );
