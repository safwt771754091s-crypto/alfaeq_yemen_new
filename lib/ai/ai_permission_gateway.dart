...
class AlfaeqAiPermissionGateway {
  ...
  static const Map<String, _AiActionPolicy> _policies = {
    ...
    'get_my_orders': _AiActionPolicy(level: AiActionLevel.read, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'get_my_order': _AiActionPolicy(level: AiActionLevel.read, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'get_my_account_summary': _AiActionPolicy(level: AiActionLevel.read, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'get_security_summary': _AiActionPolicy(level: AiActionLevel.read, roles: <String>{'developer', 'admin', 'owner'}, requiresSignIn: true),
    'get_my_cart': _AiActionPolicy(level: AiActionLevel.read, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'add_to_cart': _AiActionPolicy(level: AiActionLevel.reversible, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'update_cart_item': _AiActionPolicy(level: AiActionLevel.reversible, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'remove_from_cart': _AiActionPolicy(level: AiActionLevel.reversible, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
    'create_order_draft': _AiActionPolicy(level: AiActionLevel.reversible, roles: <String>{'customer', 'merchant', 'driver', 'developer', 'admin', 'owner'}, requiresSignIn: true),
  };
  ...
  Future<AiPermissionDecision> authorize(String action, {bool userConfirmed = false}) async {
    ...
    if (!policy.roles.contains(role)) return AiPermissionDecision.denied('ليس لديك صلاحية لتنفيذ هذه العملية.', role: role);
    if (policy.level != AiActionLevel.read && !userConfirmed) {
      return AiPermissionDecision.confirmationRequired('هذه العملية تتطلب تأكيداً صريحاً قبل التنفيذ.', role: role, level: policy.level);
    }
...
class _AiActionPolicy {
  ...
  const _AiActionPolicy({required this.level, required this.roles, required this.requiresSignIn});
}
